...
test('Query Parameters using playwright & typescript using faker', async ({ request }) => {
...
const getAPIResponseUsingQueryParams = await request.get('/booking', {
    params: {
        firstname: firstName,
        lastname: lastName
    }
...
expect(getAPIResponseUsingQueryParams.status()).toBe(200)
expect(getAPIResponseUsingQueryParams.statusText()).toBe('OK')

expect(getAPIResponseUsingQueryParams.headers()['content-type'])
    .toContain('application/json')

const getAPIQueryResponse = await getAPIResponseUsingQueryParams.json()

console.log(
    'GET API Response using Query Parameters:',
    JSON.stringify(getAPIQueryResponse, null, 2)
)
